"""
Core agent runtime - shared between CLI, TUI, and library modes
"""
...
class AgentRuntime:
    ...
    async def execute_tool(self, tool: types.AgentTools, depth: int = 0) -> str:
        ...
        if tool.action == "Agent":
            ...
            # Import tool executor dynamically to avoid circular imports
            from ..tools.registry import execute_tool
            return await execute_tool(tool, self.state.working_dir)
    ...
    # @trace
    async def run_loop(self, user_message: str, max_iterations: int = 20, depth: int = 0) -> str:
